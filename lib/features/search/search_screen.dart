  ...
  Widget build(BuildContext context) {
    ...
    return CustomScrollView(
      ...
      slivers: [
          ...
          SliverList(
            delegate: SliverChildBuilderDelegate(
              (context, index) {
                ...
                return _RecentTile(
                  query: q,
                  onTap: () => widget.onRecentTap(q),
                  onRemove: () => widget.onRecentRemove(q),
                )
                    .animate()
                    .fadeIn(
        ...
        // ── Browse Categories ─────────────────────────────
        _SliverBrowseGrid(onTap: widget.onCategoryTap),
...
class _SliverBrowseGrid extends StatelessWidget {
  final ValueChanged<String> onTap;
  const _SliverBrowseGrid({required this.onTap});
      ...
      slivers: [
        ...
        SliverPadding(
          ...
          sliver: SliverGrid(
            ...
            delegate: SliverChildBuilderDelegate(
              (context, i) {
                if (i >= _SliverBrowseGrid._cats.length) return null;
                return _CategoryCard(
                  cat: _SliverBrowseGrid._cats[i],
                  delay: i * 45,
                  onTap: () => onTap(_SliverBrowseGrid._cats[i].query),
                );
              },
              childCount: _SliverBrowseGrid._cats.length,
            ),
          ),
        ),
      ],
      ...
      slivers: [
        results.when(
          ...
          data: (songs) {
            ...
            final isEnglishSearch = songs.take(8).any((s) => 
                s.language.toLowerCase().contains('english'));

            return SliverMainAxisGroup(
              slivers: [
                if (isEnglishSearch)
                  SliverToBoxAdapter(
                    child: _EnglishSupportBanner(),
                  ),
                SliverToBoxAdapter(
                  child: Padding(
  ...
  Widget _buildShimmerItem() {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        duration: 1400.ms, color: Colors.white.withOpacity(0.045));
  }
...
// ─── ENGLISH SUPPORT BANNER ───────────────────────────────────

class _EnglishSupportBanner extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
    return Container(
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 2),
                Text(
                  "Early access is live! We're currently expanding our English library to bring you the full experience soon.",
                  style: TextStyle(
